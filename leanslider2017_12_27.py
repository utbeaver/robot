...
class SliderMechanismC(SliderJoint):
	...
	Measure(self, "ActForce", "-MOTION(%s, 0, 4, %s)"%(transJ.motionName(), braceGeo.name()))
        ...
        (JF, marIF, marJF)=createJ(self, "slider_linkF", slider, LinkLF, pLnkLF_I, (0,0,0), self.basemarJ) 
	...
	Measure(self, "LeftForceII", "-JOINT(%s, 0, 2, %s)-JOINT(%s, 0, 2, %s)"%(JF.name(), marJF.name(), JB.name(), marJB.name()))
        ...
        p2_p3_sR=(dl_+delta+R_*np.cos(-alpha34)*.77/.77, R_+R_*np.sin(-alpha34)*.77/.77, temp+T2)
...
#
  ...
  for i in paraCompf1: i.visual("on")
...
#
  ...
  createPlate(DeltaB3, "triangleB", temp, lm22.r, lm22.T, mar2)
...
#
  ...
  (J, mI, mJ)=createJ(model, "J27", part5, DeltaB4, (p7[2], 0,0), (0,0,0), mar3)
  Motion123(model, "motion3", mI, mJ, "(90*sin(TIME/5.0*PI*2))*%dd"%_motion3) 
  mar5=Marker(part5, "mar5",  (0,0,0), (0,90,0), ref=mar4)
  length=(p7[1]-p5[1])*2

  mar6=Marker(part5, "mar6",  (p7[2]/1.414+length/2,0,0), (0,0,0), ref=mar5)
  ratio=0.5
  lm33=SliderMechanismC(model, "SLM33", dl*R2*ratio, dr*R2*ratio, L*ratio*0.6, L*ratio*0.2,  R2*ratio, mar6, alpha1,shift1=shift1, shift2=shift2, _motion=_motion4, vis="on", has1stParallel=0, has2ndParallel=0)

  xy=pcir_pnts(lm22.r, 210-90, 330-90)+[(p7[2]/1.414, -lm33.Wi-lm33.T*5), (p7[2]/1.414, lm33.Wi+lm33.T*5)]
