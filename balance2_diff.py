...
def Sprocket(part, name_, R, N, baeline, T, p, mar_):
    ...
    Extrusion(part, name_+"4", [(R/2*np.cos(i), R/2*np.sin(i)) for i in np.linspace(0, 330, 10)*d2r], baeline+T, T*1.01, "RED", mar) 
    return (mar, r, L)
...
class SliderMechanismC(SliderJoint):
    def __init__(self, parent, Name, dl_, dr_, L_, L1_, R_,  mar1, alpha1=0, shift1=0, shift2=0, _motion=1, ball=0, inner=0, quad=0, vis="on", LW=0):
	...
	r=0.1*R_
        ...
        braceGeo1=Marker(self.Brace, "braceGeo1", (0,0,0), (alpha12+self.alpha/d2r,0,0), ref=self.mar1)
        LL=np.sqrt(L_*L_+R_*R_)
        RR=R_/2
        w_3=braceH/3
        W=braceH/2
        pnts1=pcir_pnts(RR, 90, 270)+[(LL-RR, -RR), (LL-RR, RR)]
        pnts1=pcir_pnts(RR, 90, 270, xyz=(-LW, 0, 0))+[(L1_-RR, -RR*0.5), (L1_-RR, RR*0.5)]
        pnts2=[(RR*0.9, RR*0.9), (RR*0.9,-RR*0.9)]+pcir_pnts(RR*0.5, -90, 90, xyz=(LL, 0, 0))
        Extrusion(self.Brace, "back",    pnts1, -W, T,    "YELLOW", braceGeo1)
        Extrusion(self.Brace, "front",   pnts1, W-T, T, "YELLOW", braceGeo1)
        Extrusion(self.Brace, "middle",  pnts2, -(W-T), 2*(W-T), "YELLOW", braceGeo1)
	...
	Measure(self, "ActForce", "-MOTION(%s, 0, 4, %s)"%(transJ.motionName(), braceGeo.name()))
        ...
        pnts1=pcir_pnts(RR, 90, 270, xyz=(0, 0, 0))+[(L1_-RR, -RR*0.5), (L1_-RR, RR*0.5)]
        pnts2=[(RR*0.9, RR*0.9), (RR*0.9,-RR*0.9)]+pcir_pnts(RR*0.5, -90, 90, xyz=(L1_, 0, 0))
        crankGeo=Marker(crankFF, "geoMar", (LL, 0, 0), (self.alpha2nd/d2r-alpha12-self.alpha/d2r,0,0), ref=braceGeo1)
        Extrusion(crankFF, "back",    pnts1, -W, T,    "YELLOW", crankGeo)
        Extrusion(crankFF, "front",   pnts1, W-T, T, "YELLOW", crankGeo)
        Extrusion(crankFF, "middle",  pnts2, -(W-T), 2*(W-T), "YELLOW", crankGeo)
        temp=W+T/2+3*T
        cr_p1=(L_, R_, temp)
	...
	self.alpha21=np.arctan2(R_, dl_+delta)
        ...
        for i in self.getKids("Part"): 
            if "CrankFF" not in i.name() and "Brace" not in i.name():
                i.visual(self.vis)
...
if __name__ == "__main__":	
  ...
  shift1=-20
  shift2=-100.0
  lm11=SliderMechanismC(model, "SLM11", dl*R, dr*R, L, L,  R, mar1, alpha1,shift1=shift1, shift2=shift2, vis="off", LW=(0.5*L*0))
...
#
  ...
  temp=pcir_pnts(R/2, 0, 180)+[(-R/2, -L*2), (R/2, -L*2)]
  temp=[(i[0], i[1], WT11_15*0) for i in temp]
  Extrusion(ground, "triangleF", temp,  WT11_15-lm11.T/2, lm11.T, ref=mar1)
  Extrusion(ground, "triangleB", temp,  -WT11_15-lm11.T/2, lm11.T, ref=mar1)
  ...
  Extrusion(ground, "cyn2", cir_pnts(SR/2), -WT11_15-lm11.T, 2*WT11_15+2*lm11.T, ref=mar1)  
  Extrusion(ground, "cyn3", cir_pnts(R), -2.1*L, 0.1*L, ref=Marker(ground, "cyn3_mar", (0,0,0), (0, -90, 0), ref=mar1))
  Extrusion(ground, "cyn4", cir_pnts(R*1.5), -2.3*L, 0.2*L, ref=Marker(ground, "cyn4_mar", (0,0,0), (0, -90, 0), ref=mar1))
...
#
  ...
  mar3pos=(mar2pos[0]+dxyz3[0], mar2pos[1]+dxyz3[1], 0) 
...
#balance mechanism
  ...
  pnts=Sprocket_Pair("chain_f01", SR, 24, idlerf1, lm22.crank, lm22.Brace, upnt6,  mar3pos, lm11.Wi+lm11.T+lm11.T*2*0+offset, lm11.T, mar1, right=0) 
  #l=zip(*pnts)
  #plt.plot(l[0], l[1])
  ...
  Sprocket_Pair("chain_f12", SR, 24, idlerf2, idlerf1, lm11.crank, upnt3,  upnt6, lm11.Wi+lm11.T*2+lm11.T*2*0+offset, lm11.T, mar1, right=0) 
  #createJ(model, "idlerf2_crank", idlerf2, lm11.crank, Z(upnt3, lm11.Wi-lm11.T), (0,0,0), mar1) 
  Extrusion(lm11.crank, "axis", cir_pnts(SR*0.5, upnt6),  -lm11.Wi-lm11.T*0, 2*(lm11.Wi+lm11.T*2)-lm11.T, "RED", Marker(lm11.crank, "cyn_mar1", ref=mar1))

  idlerb2=Part(model, "idlerb2")
  Sprocket_Pair("chain_b12", SR, 24, idlerb2, idlerb1, lm11.crank, upnt3,  upnt6, -lm11.Wi-lm11.T*1-offset, lm11.T, mar1) 
  #createJ(model, "idlerb2_crank", idlerb2, lm11.crank, Z(upnt3, -lm11.Wi-lm11.T), (0,0,0), mar1) 

  L1=0.5*L
  R1=L1/2
  a_=lm11.alpha1st+lm11.alpha+np.pi
  wpnt1=(L1*np.cos(a_)*0, -L1+L1*np.sin(a_)*0, 0)
  ...
  Sprocket_Pair("chain_f23", SR, 24, idlerf3, idlerf2, lm11.Brace, wpnt1,  upnt3, lm11.Wi+lm11.T+lm11.T*2+offset, lm11.T, mar1, right=0) 
  #(J, idlerf3I, idlerf3J)=createJ(model, "idlerf3_lm11", idlerf3, lm11.Brace, Z(wpnt1, lm11.Wi+lm11.T), (0,0,0), mar1) 

  Sprocket_Pair("chain_f23_", SR, 24, bal2, lm11.crank, lm11.Brace, wpnt1,  upnt3, lm11.Wi+lm11.T*0+offset/2, lm11.T, mar1, left=0, right=0) 

  bal3=Part(model, "bal3")
  idlerb3=Part(model, "idlerb3")
  Sprocket_Pair("chain_b23", SR, 24, idlerb3, idlerb2, lm11.Brace, wpnt1,  upnt3, -lm11.Wi-lm11.T*2-offset, lm11.T, mar1, right=0) 
  #(J, idlerb3I, idlerb3J)=createJ(model, "idlerb3_lm11", idlerb3, lm11.Brace, NZ(wpnt1, lm11.Wi+lm11.T), (0,0,0), mar1) 
  Extrusion(bal2, "axis", cir_pnts(SR*0.5, wpnt1),  -lm11.Wi-lm11.T*3, 2*(lm11.Wi+lm11.T*3)+lm11.T, "RED", Marker(bal2, "cyn_mar1", ref=mar1))
  Extrusion(lm11.Brace, "axis", cir_pnts(SR*0.5, upnt3),  -lm11.Wi-lm11.T*2, 2*(lm11.Wi+lm11.T*3), "RED", Marker(lm11.Brace, "cyn_mar1", ref=mar1))


  L2=0.5*L
  R2=L2/3
  Extrusion(bal2, "cyn3", pcir_pnts(R1, 90, 270)+pcir_pnts(R2, -90, 90, xyz=(L2, 0, 0)), -(lm11.Wi-lm11.T), 2*(lm11.Wi-lm11.T), "RED", GeoMarBal2)
  wpnt2=Xform((L2, 0, 0), (np.pi+lm11.alpha2nd)/d2r, wpnt1)

  idlerf4=Part(model, "idlerf4")
  Sprocket_Pair("chain_f34", SR, 24, idlerf4, idlerf3, bal2, wpnt2,  wpnt1, lm11.Wi+lm11.T*2+lm11.T*2*0+offset, lm11.T, mar1, right=0) 
  ...
  Sprocket_Pair("chain_b34", SR, 24, bal3, idlerb3, bal2, wpnt2,  wpnt1, -lm11.Wi-lm11.T*3-offset, lm11.T, mar1, left=0, right=0) 
  Extrusion(bal3, "cyn", cir_pnts(SR*0.7, wpnt2),  -lm11.Wi-lm11.T*2, lm11.T, "RED", Marker(bal3, "cyn_mar", ref=mar1))
  Extrusion(bal3, "axis", cir_pnts(SR*0.5, wpnt2),  -lm11.Wi-lm11.T*1, 2*(lm11.Wi+lm11.T*3)-lm11.T, "RED", Marker(bal3, "cyn_mar1", ref=mar1))
  ...
  Sprocket_Pair("chain_f45", SR, 24, bal4, idlerf4, bal3, wpnt3,  wpnt2, lm11.Wi+lm11.T+lm11.T*2*0+offset, lm11.T, mar1, left=0, right=0) 
#balance mechanism
